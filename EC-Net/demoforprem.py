...
def main(num,epoch):
    ...
    src_points = np.load('demo/DePoLL-prem/video_reconstructionBeforeRigid_scaled100_spa0015_'+num+'.npy')
    ref_points = np.load('demo/DePoLL-prem/surface_CT_scaled100_spa0015_'+num+'.npy')

    state_dict = torch.load('EC-Net.pth.tar')
    ...
    transform = np.load('demo/gtscaled100_' + num + '_V2S.npy')
    data_dict["transform"] = transform.astype(np.float32)

    neighbor_limits = [37, 32, 30, 34]  # default setting in 3DMatch Calibrate neighbors: [68 28 31 31].
