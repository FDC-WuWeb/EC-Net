...
class LocalDownSample(nn.Module):
    def __init__(self, channels, sigma_t=0.02):  # 引入 sigma_t
        super(LocalDownSample, self).__init__()
        # self.npts_ds = npts_ds
        self.K = 4
        self.group_type = 'diff'
        ...
        self.sigma_t = sigma_t  # 记录阈值

    def forward(self, x, npts_ds):
        neighbors = ops.group(x, self.K, self.group_type)
        q = self.q_conv(rearrange(x, 'B C N -> B C N 1')).contiguous()
        q = rearrange(q, 'B C N 1 -> B N 1 C').contiguous()
        k = self.k_conv(neighbors)
        k = rearrange(k, 'B C N K -> B N C K').contiguous()
        v = self.v_conv(neighbors)
        v = rearrange(v, 'B C N K -> B N K C').contiguous()
        ...
        # 计算标准差 sigma
        sigma = rearrange(torch.std(attention, dim=-1, unbiased=False), 'B N 1 -> B N').contiguous()

        # 【核心修改点】：根据阈值识别边缘点，并按你的逻辑使其权重倍增（*2）
        # 这里巧妙利用了 topk 的特性：只要 sigma_t 不太大，高分点的顺序不会改变，结果就不会变
        edge_mask = (sigma > self.sigma_t).float()
        selection = sigma * (1.0 + edge_mask)  # 大于 sigma_t 的点，其 selection 分数直接 x2

        self.idx = selection.topk(npts_ds, dim=-1)[1]
        scores = torch.gather(attention, dim=1, index=repeat(self.idx, 'B M -> B M 1 K', K=attention.shape[-1]))
        v = torch.gather(v, dim=1, index=repeat(self.idx, 'B M -> B M K C', K=v.shape[-2], C=v.shape[-1]))
        out = rearrange(scores @ v, 'B M 1 C -> B C M').contiguous()
        return out
...
class KPConvFPN(nn.Module):
    ...
    def forward(self, feats, data_dict):
        ...
        # print(subsampling_list[0].shape)
        # print(subsampling_list[1].shape)
        # print(subsampling_list[2].shape)
        # print(points_list[3].shape) #
        # print(points_list[2].shape) #
        # print(points_list[1].shape) #
        # print(points_list[0].shape) #
        # for i in range(4):
        #     # 将 points_list[i] 转换为 numpy 数组，并确保为 float64 类型
        #     points_array = np.asarray(points_list[i].cpu(), dtype=np.float64)
        #
        #     # 检查数据的形状是否为 N x 3
        #     if points_array.shape[1] != 3:
        #         raise ValueError(f"points_list[{i}] should have shape N x 3, but got shape {points_array.shape}")
        #
        #     # 转换为 Open3D 点云格式并保存为 ply 文件
        #     point_cloud = o3d.geometry.PointCloud()
        #     point_cloud.points = o3d.utility.Vector3dVector(points_array)
        #
        #     filename = f"points_list_{i}.ply"
